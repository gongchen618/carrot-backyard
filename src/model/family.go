...
type FamilyInterface interface {
	...
	DeleteOneFamilyMemberByStudentID(studentID string) (param.FamilyMember, error)
...
func (m *model) DeleteOneFamilyMemberByStudentID(studentID string) (param.FamilyMember, error) {
	filter := bson.M{"student_id": studentID}
	deletedMember := param.FamilyMember{}
	err := m.familyCollection().FindOneAndDelete(m.context, filter).Decode(&deletedMember)
	if err != nil {
