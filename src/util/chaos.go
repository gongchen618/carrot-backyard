...
func SendSameMessageToManyFriends(message string, people []param.PersonWithQQ) []param.PersonWithQQ {
	...
	for _, person := range people {
		...
		newMessage := getMessageLinkMixedVersion(message)
		newMessage = getMessageChaosVersion(newMessage)
		newMessage = GetHitokotoWarpedMessage(newMessage)

		status := QQSendAndFindWhetherSuccess(person.QQ, newMessage)
		if status == false {
